...
下图直接由**真实 P110 EXP2 磁信号和应变片推导应力**生成，不是合成 Demo。绘图仅裁除检测管内区间首尾各1%的边缘效应，冻结特征表没有被修改。

![真实P110磁信号与应变案例](docs/results/real_p110_magnetic_case.png)

下图来自**真实 P110 EXP2 实验数据**，不是合成 Demo。图中选择通过主特征质量门禁、同时具有6点与12点数据的单侧开槽 MEM 第一轮，展示真实 X 轴磁信号以及与应变片推导轴向应力的对应关系。真实数据可以看到零载/加载后的分布变化，但不能据此宣称每个加载阶段都严格单调。
